...
freom selenium.webdriver.common.by import By
...
#Selecionar a tarifa e chamar taxi
...
close_button_card = (By.CSS_SELECTOR, '.payment-picker.opne .close-button')
confirm_card = (By.CSS_SELECTOR, ' .pp-value-text')

#Adicionar Comentario
add_coment =  (By.ID, 'comment')
switch_blanket = (By.CSS_SELECTOR, '.switch')
switch_blanket_active = (By.CSS_SELECTOR, '#root > div > div.worflow > div.workflow-subcontainer > div.tariff-picker.show > div.form > div.reqs.open > div.reqs-body >')
...
def click_comfort_icon(self):
    self.driver.find_element(*self.comfort_ion_locator).click()
...
def click_number_text(self,Telefone):
    ...
    code_input = WebDriverWait(self.driver, timeout=3).until(EC.visibility_of_element_located(self.number_code))
    code_input.clear()
    code_input.send_keys(code)

    self.driver.find_element(*self.code_confirm).click()

def numero confirmado(set):
